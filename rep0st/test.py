...
from rep0st.framework.flags import DeclareStringFlag, do

MegaFlag = DeclareStringFlag('mega_flag', 'esdfsdf', 'This is the mega flag')


class Main:

  @inject
  def __init__(self, mega_flag: MegaFlag):
    self.mega_flag = mega_flag

  def getvalue(self):
    return f"yo {self.mega_flag.get()}"


class MainModule(Module):

  def configure(self, binder: Binder) -> None:
    binder.bind(MegaFlag)
    binder.bind(Main)
...
injector = do([MainModule, OtherModule])

print(injector.get(Main).getvalue())

fv = injector.get(FlagValues)
print(fv)
